...
- **角色设定图** — **每个角色一张**：16:9 分三区，左侧约 34% 证件照式半身像（面部基准）、右上全身三视图、右下关键细节特写条。白底方便抠图，走 codex 内置出图（可选）
...
- **同一批角色的画风可能有差异**——各自独立出图。早期用「扁平矢量卡通」时漂得很厉害（同批出成动画感／半写实／水墨写实三种）。把画风写死进每条提示词能压住一部分，代价是换风格要逐条改，而且跟出图时选的那一档正面打架——所以现在画风是整批共用的一段指令，由调用方附加在提示词前面。漂移仍不能完全排除，在意的话拿第一张当参考图压一压，见 `references/sheet.md`
