...
同一批角色各自独立出图，**画风可能有差异**——同一批出成动画感／半写实／水墨写实三种，摆在一起不像同一部片子。**压住它的办法不是往提示词里写画风**：那样每条提示词都被钉死在一档上，换风格要逐条改，而漂移照旧（模型对同一句风格描述的理解本来就不稳）。正确的做法是**整批共用同一段风格指令**，由调用方在出图时统一附加在提示词前面。
...
代价是第一张的画风就定了全片基调，出得不好就得重来。用户在意统一性就上参考图，只是要几张草图就不必。参考图和统一的风格指令是两道保险，不冲突：前者锁住这一批的实际成像，后者决定这一批往哪个方向出。
