...
5. `image.prompt` 是**单张表现性插画**（不是技术图，可以放开打光）：四分之三视角半身、纯中性背景、柔和方向主光 + 冷调补光、浅景深、面部最实。
   ...
   **身份必须落实成可见设计，不能只留在 `persona.identity`。** 先从观察记录确定人物在本次造型时的身份、经济条件、职业、年龄和出场场合，再选择与年代相符的服装轮廓、面料与维护状态、剪裁层次、发式配饰、鞋履和体态。`image.prompt`、`image.promptLocal` 与 `image.sheet` 的主体描述必须保留同一组身份线索；不能一处写贵族继承人，另一处只剩泛泛的年轻读书人。
   ...
   **服装写具体材质与使用状态，不写渲染质量。** 保养良好、挺括平整、细密织物与精细缝边同样是生活细节。推断的服装设计按第 1 条标注；不要把推断写成原文事实，也不要把平时衣着和特殊场合装束拼成一套。
