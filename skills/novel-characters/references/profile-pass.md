...
4. **族裔、年代、地域必须从原文推断出来，明确写进 `image.prompt` 和 `image.sheet`。**
   ...
   **线索实在不够时默认东亚，不要留空。** 留空等于选了西方白人——模型的先验就在那儿，不写它就替你写了。但默认之后仍然要落到能画的描述上：写 `East Asian, Han Chinese features, monolid eyes`，不要写泛泛的「亚洲人」`Asian`——那既没有五官也没有地域，模型会在东亚、南亚、东南亚之间自己挑，同一批角色挑出三种脸。
...
5. `image.prompt` 是**单张表现性插画**（不是技术图，可以放开打光）：四分之三视角半身、纯中性背景、柔和方向主光 + 冷调补光、浅景深、面部最实。
   ...
   **服装写它的来历，不写它的材质渲染**：料子、剪裁、年代和地域已经在第 4 条里定了；这里补的是**这一件**的状态——肘部袖口膝盖的磨损、补丁、洗得发白、系法和穿戴习惯。谁穿它、穿了多久，比「织纹可见」更能让它像真衣服。
