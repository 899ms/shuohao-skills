...
同一部剧的所有场景应该像同一个美术组画的。这靠两件事：表面处理句是写死的一套（见 `scene-pass.md` 第 5 条），画风则由调用方在出图时整批附加同一段指令——**都不写进单条提示词**。压不住的话再拿第一个主场景的成图当参考喂给后面的场景（同样走 `-i` + stdin）。代价与角色 skill 相同：第一张定基调，出得不好就得重来。
